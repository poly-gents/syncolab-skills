...
description: "Implement reusable UI components with proper testing"
---
...
Implement reusable UI components with proper testing

This skill provides operational guidance for Build Components, including tool usage patterns, workflows, and quality expectations aligned with Syncolab skill standards.

## When to Use

- Use when the user needs help with Build Components.
- When integrations for this domain are available and the task matches the workflows below.

## When NOT to Use

- When the task is unrelated to Build Components or covered by a more specific skill.
...
1. Confirm the request maps to Build Components and required tools are available.
...
- Legacy content migrated from `skills/old_skills.json` (`build-components`).
