...
description: "Hand off calls with clean transcripts and summaries"
---
...
Hand off calls with clean transcripts and summaries

This skill provides operational guidance for Escalate with Summary, including tool usage patterns, workflows, and quality expectations aligned with Syncolab skill standards.

## When to Use

- Use when the user needs help with Escalate with Summary.
- When integrations for this domain are available and the task matches the workflows below.

## When NOT to Use

- When the task is unrelated to Escalate with Summary or covered by a more specific skill.
...
1. Confirm the request maps to Escalate with Summary and required tools are available.
...
- Legacy content migrated from `skills/old_skills.json` (`escalate-with-summary`).
