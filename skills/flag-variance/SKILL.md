---
name: flag-variance
description: "Alert on spend variances and budget anomalies"
---
...
Alert on spend variances and budget anomalies

This skill provides operational guidance for Flag Variance, including tool usage patterns, workflows, and quality expectations aligned with Syncolab skill standards.

## When to Use

- Use when the user needs help with Flag Variance.
- When integrations for this domain are available and the task matches the workflows below.

## When NOT to Use

- When the task is unrelated to Flag Variance or covered by a more specific skill.
...
1. Confirm the request maps to Flag Variance and required tools are available.
...
- Legacy content migrated from `skills/old_skills.json` (`flag-variance`).
