---
name: notion
description: Automates Notion pages, databases, blocks, comments, and users via Rube MCP (Composio). Use after RUBE_SEARCH_TOOLS and an ACTIVE Notion connection with schema-driven tool slugs.
---
...
- Every call preceded by `RUBE_SEARCH_TOOLS` for live parameters.
...
1. Confirm Rube MCP (`RUBE_SEARCH_TOOLS` responds).
2. `RUBE_MANAGE_CONNECTIONS` with toolkit `notion`; complete OAuth if not ACTIVE.
3. `RUBE_SEARCH_TOOLS` for the Notion action; execute with schema-exact fields.
4. Prefer list/search tools to resolve ids before destructive updates.
5. Report results with ids and URLs from Composio responses.
...
- Legacy content migrated from `skills/old_skills.json` (`notion`).
