...
description: "Upgrade Pulumi providers safely without unintended infra changes."
---
...
Upgrade Pulumi providers safely without unintended infra changes.

This skill provides operational guidance for Pulumi Provider Upgrade, including tool usage patterns, workflows, and quality expectations aligned with Syncolab skill standards.

## When to Use

- Use when the user needs help with Pulumi Provider Upgrade.
- When integrations for this domain are available and the task matches the workflows below.

## When NOT to Use

- When the task is unrelated to Pulumi Provider Upgrade or covered by a more specific skill.
...
1. Confirm the request maps to Pulumi Provider Upgrade and required tools are available.
...
- Legacy content migrated from `skills/old_skills.json` (`provider-upgrade`).
