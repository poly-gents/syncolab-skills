...
description: "Automate provider repository upgrade workflows."
---

# Pulumi Upgrade Provider Repo

## Purpose

Automate provider repository upgrade workflows.

This skill provides operational guidance for Pulumi Upgrade Provider Repo, including tool usage patterns, workflows, and quality expectations aligned with Syncolab skill standards.

## When to Use

- Use when the user needs help with Pulumi Upgrade Provider Repo.
- When integrations for this domain are available and the task matches the workflows below.

## When NOT to Use

- When the task is unrelated to Pulumi Upgrade Provider Repo or covered by a more specific skill.
...
1. Confirm the request maps to Pulumi Upgrade Provider Repo and required tools are available.
...
- Legacy content migrated from `skills/old_skills.json` (`pulumi-upgrade-provider`).
