...
description: "Execute diagnostic scripts and gather system telemetry"
---
...
Execute diagnostic scripts and gather system telemetry

This skill provides operational guidance for Run Diagnostics, including tool usage patterns, workflows, and quality expectations aligned with Syncolab skill standards.

## When to Use

- Use when the user needs help with Run Diagnostics.
- When integrations for this domain are available and the task matches the workflows below.

## When NOT to Use

- When the task is unrelated to Run Diagnostics or covered by a more specific skill.
...
1. Confirm the request maps to Run Diagnostics and required tools are available.
...
- Legacy content migrated from `skills/old_skills.json` (`run-diagnostics`).
