---
name: shadow-canary-watcher
description: "Post-deploy monitoring — canary metrics, error budgets, and rollback triggers."
---

# Shadow Canary Watcher

## Purpose

Post-deploy monitoring — canary metrics, error budgets, and rollback triggers.

This skill provides operational guidance for Shadow Canary Watcher, including tool usage patterns, workflows, and quality expectations aligned with Syncolab skill standards.

## When to Use

- Use when the user needs help with Shadow Canary Watcher.
- When integrations for this domain are available and the task matches the workflows below.

## When NOT to Use

- When the task is unrelated to Shadow Canary Watcher or covered by a more specific skill.
...
1. Confirm the request maps to Shadow Canary Watcher and required tools are available.
...
- Define success metrics before increasing traffic
- Watch: error rate, latency, saturation, business KPIs
- Pre-agree rollback thresholds and owner

Output: watch brief + escalate if thresholds breach.
...
- Legacy content migrated from `skills/old_skills.json` (`shadow-canary-watcher`).
