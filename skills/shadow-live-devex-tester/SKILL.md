---
name: shadow-live-devex-tester
description: "Hands-on contributor journey test — clone, install, test, and first PR path."
---

# Shadow Live DevEx Tester

## Purpose

Hands-on contributor journey test — clone, install, test, and first PR path.

This skill provides operational guidance for Shadow Live DevEx Tester, including tool usage patterns, workflows, and quality expectations aligned with Syncolab skill standards.

## When to Use

- Use when the user needs help with Shadow Live DevEx Tester.
- When integrations for this domain are available and the task matches the workflows below.

## When NOT to Use

- When the task is unrelated to Shadow Live DevEx Tester or covered by a more specific skill.
...
1. Confirm the request maps to Shadow Live DevEx Tester and required tools are available.
...
Execute the **documented** path only (no insider shortcuts).

Log every friction point with:
- Step, expected, actual, suggested fix

Measure time to: install deps, run tests, run app locally, push branch.

Output: DX report with quick wins vs structural fixes.
...
- Legacy content migrated from `skills/old_skills.json` (`shadow-live-devex-tester`).
