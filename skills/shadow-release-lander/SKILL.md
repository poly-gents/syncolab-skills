---
name: shadow-release-lander
description: "Land-and-deploy discipline — merge, deploy sequence, verification, and rollback readiness."
---

# Shadow Release Lander

## Purpose

Land-and-deploy discipline — merge, deploy sequence, verification, and rollback readiness.

This skill provides operational guidance for Shadow Release Lander, including tool usage patterns, workflows, and quality expectations aligned with Syncolab skill standards.

## When to Use

- Use when the user needs help with Shadow Release Lander.
- When integrations for this domain are available and the task matches the workflows below.

## When NOT to Use

- When the task is unrelated to Shadow Release Lander or covered by a more specific skill.
...
1. Confirm the request maps to Shadow Release Lander and required tools are available.
...
- Confirm artifact and config for target environment
- Order: migrations → services → traffic shift (as applicable)
- Post-deploy smoke checklist
- Rollback drill: know the command and data implications

Output: go-live checklist with sign-offs.
...
- Legacy content migrated from `skills/old_skills.json` (`shadow-release-lander`).
