...
1. Confirm Rube MCP available (`RUBE_SEARCH_TOOLS`).
2. `RUBE_MANAGE_CONNECTIONS` with toolkit `slack`; OAuth until ACTIVE.
3. `RUBE_SEARCH_TOOLS` for Slack action; call with schema parameters.
4. Resolve channel/user ids when the user gives names.
5. Confirm sends in #channels when the user asked for a dry run or preview.
...
- Legacy content migrated from `skills/old_skills.json` (`slack`).
