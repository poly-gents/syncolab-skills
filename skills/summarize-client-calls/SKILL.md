---
name: summarize-client-calls
description: "Capture and summarize key points from client conversations"
---
...
This skill provides operational guidance for Summarize Client Calls, including tool usage patterns, workflows, and quality expectations aligned with Syncolab skill standards.

## When to Use

- Use when the user needs help with Summarize Client Calls.
- When integrations for this domain are available and the task matches the workflows below.

## When NOT to Use

- When the task is unrelated to Summarize Client Calls or covered by a more specific skill.
...
1. Confirm the request maps to Summarize Client Calls and required tools are available.
...
- Legacy content migrated from `skills/old_skills.json` (`summarize-client-calls`).
