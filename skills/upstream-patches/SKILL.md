...
description: "Manage upstream Terraform patch stacks in provider repos."
---
...
This skill provides operational guidance for Pulumi Upstream Patches, including tool usage patterns, workflows, and quality expectations aligned with Syncolab skill standards.

## When to Use

- Use when the user needs help with Pulumi Upstream Patches.
- When integrations for this domain are available and the task matches the workflows below.

## When NOT to Use

- When the task is unrelated to Pulumi Upstream Patches or covered by a more specific skill.
...
1. Confirm the request maps to Pulumi Upstream Patches and required tools are available.
...
- Legacy content migrated from `skills/old_skills.json` (`upstream-patches`).
